...
const login = async (req, res) => {
    ...
    try {
        ...
        // 🔹 If the password matches the stored LinkedIn sub, allow login
        ...
        // 🔹 If not LinkedIn, verify password using bcrypt
...
const checkUserExists = async (req, res) => {
    try {
        const account = await Account.getAccountByEmail(req.params.email);
        res.status(200).json({ exists: !!account }); // Return true if account exists, false otherwise
    } catch (error) {
