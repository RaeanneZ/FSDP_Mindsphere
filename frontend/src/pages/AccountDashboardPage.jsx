...
const AccountDashboardPage = () => {
    ...
    const { accountService, childrenService, programmeService } =
        backendService;
        ...
        const fetchData = async () => {
            try {
                ...
                const fetchedBookingData =
                    await programmeService.getRegisteredProgrammesByAccount(
                        email
                    );
                ...
                // Fetch children data
                const fetchedChildrenData =
                    await childrenService.getChildByEmail(email);
                //setChildren(fetchedChildrenData); // Set the fetched children data to state

                // Create an array to hold original children data
                const originalChildrenArray = fetchedChildrenData.map(
                    (child) => ({
                        dob: child.Dob,
        ...
        try {
            ...
            const childResponses = await Promise.all(
                parsedChildrenData.map(async (child) => {
                    ...
                    console.log(
                        "Gender is: ",
                        updatedFormattedChildrenData.Gender
                    );
                    console.log("DOB is: ", updatedFormattedChildrenData.Dob);
                    console.log(
                        "Needs is: ",
                        updatedFormattedChildrenData.Needs
                    );
                    console.log(
                        "School is: ",
                        updatedFormattedChildrenData.School
                    );
                    console.log(
                        "Interest is: ",
                        updatedFormattedChildrenData.Interests
                    );
            ...
            if (response) {
                console.log(
                    "Account updated successfully. Response is ",
                    response
                );
    ...
    const saveChildData = (index, data) => {
        const currentData =
            JSON.parse(sessionStorage.getItem("childData")) || [];
                    ...
                    <div className="bg-lightYellow py-10 mx-auto max-w-2xl flex flex-col items-center">
                        ...
                        >
                            ...
                            <h1 className="my-4 text-lg font-bold">
                                Enter your child&#39;s / ward&#39;s particulars
